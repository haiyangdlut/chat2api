...
class ChatService:
    ...
    async def prepare_send_conversation(self):
        ...
        self.chat_request = {
            ...
            "suggestions": [],
            "supported_encodings": [],
            "system_hints": ["picture_v2"] if self.image_gen_mode else [],
            "timezone": "America/Los_Angeles",
            "timezone_offset_min": -480,
            "variant_purpose": None if self.image_gen_mode else "comparison_implicit",
    ...
    async def upload(self, upload_url, file_content, mime_type):
        ...
        try:
            # Use aiohttp instead of curl_cffi for Azure Blob uploads.
            # curl_cffi (even without impersonate) normalizes URLs via libcurl,
            # decoding %2B to + which breaks Azure SAS signature validation.
            # aiohttp sends the URL as-is, preserving the SAS signature.
            import aiohttp
            async with aiohttp.ClientSession() as upload_session:
                async with upload_session.put(upload_url, headers=headers, data=file_content, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                    if resp.status == 201:
                        return True
                    else:
